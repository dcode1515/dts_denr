          ...
          >
            <div class="tab-vertical-icon">
              <svg
                ...
                viewBox="0 0 24 24"
                fill="none"
                :stroke="activeTab === 'in-progress' ? '#ffffff' : '#059669'"
                stroke-width="2"
              >
          ...
          >
            <div class="tab-vertical-icon">
              <svg
                ...
                viewBox="0 0 24 24"
                fill="none"
                :stroke="activeTab === 'for-release' ? '#ffffff' : '#059669'"
                stroke-width="2"
              >
                <path
          ...
          >
            <div class="tab-vertical-icon">
              <svg
                ...
                viewBox="0 0 24 24"
                fill="none"
                :stroke="activeTab === 'released' ? '#ffffff' : '#059669'"
                stroke-width="2"
              >
        ...
        <div class="tabs-vertical-body">
          ...
          <div v-show="activeTab === 'in-progress'">
            <div class="filter-controls">
              <div class="search-filter-row">
                ...
                <div class="per-page-wrapper">
                  ...
                  <select
                    v-model="inProgressPerPage"
                    @change="inProgressCurrentPage = 1"
                ...
                <div class="filter-wrapper">
                  <div class="filter-box">
                    ...
                    <select
                      v-model="inProgressDocTypeFilter"
                      @change="inProgressCurrentPage = 1"
                      class="filter-select"
                    >
                  ...
                  >
                    <li>
                      <a
                        ...
                        @click="openMyTrackingModal"
                        ><i class="bi bi-bookmark-check me-2 text-success"></i
                        >My Tracking</a
                      >
                    </li>
              ...
              <div
                ...
                v-if="inProgressSearch || inProgressDocTypeFilter"
              >
                <span class="active-filters-label">Active Filters:</span>
                <span v-if="inProgressSearch" class="filter-tag"
                  ><i class="bi bi-search"></i> "{{ inProgressSearch }}"<button
                    @click="inProgressSearch = ''"
            ...
            <div class="table-responsive">
              <table class="office-table">
                ...
                <tbody>
                  <tr
                    v-if="inProgressLoading && inProgressDocuments.length === 0"
                  >
                  ...
                  <tr
                    v-else-if="
                      !inProgressLoading && filteredInProgress.length === 0
                    "
                  >
                    <td colspan="7" class="text-center py-5">
                      <div class="empty-state">
                        ...
                        <p class="mt-2 text-muted">
                          {{
                            inProgressSearch || inProgressDocTypeFilter
                  ...
                  <tr v-for="(doc, index) in paginatedInProgress" :key="doc.id">
                    <td class="text-center">
                      <span class="row-number">{{
                        (inProgressCurrentPage - 1) * inProgressPerPage +
                        index +
                        1
                      }}</span>
                    </td>
            ...
            <div class="pagination-wrapper" v-if="inProgressTotalPages > 1">
              <div class="pagination-info">
                Showing {{ inProgressStartItem }} to {{ inProgressEndItem }} of
                {{ filteredInProgress.length }} entries
              </div>
              <div class="pagination-buttons">
                <button
                  @click="inProgressCurrentPage = 1"
                ...
                <button
                  v-for="page in inProgressDisplayedPages"
                  :key="page"
                  @click="inProgressCurrentPage = page"
                  :class="[
                    'page-btn',
                    { active: inProgressCurrentPage === page },
                  ]"
                >
                  {{ page }}
                </button>
                <button
                  @click="inProgressCurrentPage++"
                  :disabled="inProgressCurrentPage === inProgressTotalPages"
                  class="page-btn"
                >
                  <i class="bi bi-chevron-right"></i>
                </button>
                <button
                  @click="inProgressCurrentPage = inProgressTotalPages"
                  :disabled="inProgressCurrentPage === inProgressTotalPages"
                  class="page-btn"
                >
          ...
          <div v-show="activeTab === 'for-release'">
            <div class="filter-controls">
              <div class="search-filter-row">
                <div class="search-box-wrapper">
                  <div class="search-box">
                    ...
                    ><input
                      type="text"
                      v-model="forReleaseSearch"
                      @input="forReleaseCurrentPage = 1"
                      class="search-input"
                      placeholder="Search by tracking no, subject, sender..."
                    /><button
                      v-if="forReleaseSearch"
                      @click="forReleaseSearch = ''"
                      class="search-clear-btn"
                    >
                      <i class="bi bi-x-circle"></i>
                    </button>
                ...
                <div class="per-page-wrapper">
                  <span class="per-page-label">Show</span
                  ><select
                    v-model="forReleasePerPage"
                    @change="forReleaseCurrentPage = 1"
                    class="per-page-select"
                  >
                    <option :value="10">10</option>
                    <option :value="25">25</option>
                ...
                <div class="filter-wrapper">
                  <div class="filter-box">
                    <i class="bi bi-funnel filter-icon"></i
                    ><select
                      v-model="forReleaseDocTypeFilter"
                      @change="forReleaseCurrentPage = 1"
                      class="filter-select"
                    >
                      <option value="">All Document Types</option>
              ...
              <div
                class="active-filters"
                v-if="forReleaseSearch || forReleaseDocTypeFilter"
              >
                <span class="active-filters-label">Active Filters:</span
                ><span v-if="forReleaseSearch" class="filter-tag"
                  ><i class="bi bi-search"></i> "{{ forReleaseSearch }}"<button
                    @click="forReleaseSearch = ''"
                    class="filter-tag-close"
                  >
                    <i class="bi bi-x"></i></button></span
                ><span v-if="forReleaseDocTypeFilter" class="filter-tag"
                  ><i class="bi bi-funnel"></i> {{ forReleaseDocTypeFilter
                  }}<button
                    @click="forReleaseDocTypeFilter = ''"
                    class="filter-tag-close"
                  >
                    <i class="bi bi-x"></i></button></span
                ><button
              ...
              <div class="results-summary">
                <span class="results-count">{{
                  filteredForRelease.length
                }}</span>
                document(s) found in <strong>For Release</strong>
              </div>
            </div>
            <div class="table-responsive">
              <table class="office-table">
                ...
                <tbody>
                  <tr
                    v-if="forReleaseLoading && forReleaseDocuments.length === 0"
                  >
                    <td colspan="7" class="text-center py-5">
                      <div class="loader-spinner"></div>
                      <div class="mt-2">Loading documents...</div>
                  ...
                  <tr
                    v-else-if="
                      !forReleaseLoading && filteredForRelease.length === 0
                    "
                  >
                    <td colspan="7" class="text-center py-5">
                      <div class="empty-state">
                        ...
                        <p class="mt-2 text-muted">
                          {{
                            forReleaseSearch || forReleaseDocTypeFilter
                              ? "No documents match your filters"
                              : "No For Release Documents"
                          }}
                        </p>
                      </div>
                    </td>
                  ...
                  <tr v-for="(doc, index) in paginatedForRelease" :key="doc.id">
                    <td class="text-center">
                      <span class="row-number">{{
                        (forReleaseCurrentPage - 1) * forReleasePerPage +
                        index +
                        1
                      }}</span>
                    </td>
            ...
            <div class="pagination-wrapper" v-if="forReleaseTotalPages > 1">
              <div class="pagination-info">
                Showing {{ forReleaseStartItem }} to {{ forReleaseEndItem }} of
                {{ filteredForRelease.length }} entries
              </div>
              <div class="pagination-buttons">
                <button
                  @click="forReleaseCurrentPage = 1"
                  :disabled="forReleaseCurrentPage === 1"
                  class="page-btn"
                >
                  <i class="bi bi-chevron-double-left"></i></button
                ><button
                  @click="forReleaseCurrentPage--"
                  :disabled="forReleaseCurrentPage === 1"
                  class="page-btn"
                >
                  <i class="bi bi-chevron-left"></i></button
                ><button
                  v-for="page in forReleaseDisplayedPages"
                  :key="page"
                  @click="forReleaseCurrentPage = page"
                  :class="[
                    'page-btn',
                    { active: forReleaseCurrentPage === page },
                  ]"
                >
                  {{ page }}</button
                ><button
                  @click="forReleaseCurrentPage++"
                  :disabled="forReleaseCurrentPage === forReleaseTotalPages"
                  class="page-btn"
                >
                  <i class="bi bi-chevron-right"></i></button
                ><button
                  @click="forReleaseCurrentPage = forReleaseTotalPages"
                  :disabled="forReleaseCurrentPage === forReleaseTotalPages"
                  class="page-btn"
                >
                  <i class="bi bi-chevron-double-right"></i>
          ...
          <div v-show="activeTab === 'released'">
            <div class="filter-controls">
              <div class="search-filter-row">
                <div class="search-box-wrapper">
                  <div class="search-box">
                    ...
                    ><input
                      type="text"
                      v-model="releasedSearch"
                      @input="releasedCurrentPage = 1"
                      class="search-input"
                      placeholder="Search by tracking no, subject, sender..."
                    /><button
                      v-if="releasedSearch"
                      @click="releasedSearch = ''"
                      class="search-clear-btn"
                    >
                      <i class="bi bi-x-circle"></i>
                    </button>
                ...
                <div class="per-page-wrapper">
                  <span class="per-page-label">Show</span
                  ><select
                    v-model="releasedPerPage"
                    @change="releasedCurrentPage = 1"
                    class="per-page-select"
                  >
                    <option :value="10">10</option>
                    <option :value="25">25</option>
              ...
              <div
                class="active-filters"
                v-if="releasedSearch || releasedDocTypeFilter"
              >
                <span class="active-filters-label">Active Filters:</span
                ><span v-if="releasedSearch" class="filter-tag"
                  ><i class="bi bi-search"></i> "{{ releasedSearch }}"<button
                    @click="releasedSearch = ''"
                    class="filter-tag-close"
                  >
                    <i class="bi bi-x"></i></button></span
                ><span v-if="releasedDocTypeFilter" class="filter-tag"
                  ><i class="bi bi-funnel"></i> {{ releasedDocTypeFilter
                  }}<button
                    @click="releasedDocTypeFilter = ''"
                    class="filter-tag-close"
                  >
                    <i class="bi bi-x"></i></button></span
                ><button
              ...
              <div class="results-summary">
                <span class="results-count">{{ filteredReleased.length }}</span>
            ...
            <div class="table-responsive">
              <table class="office-table">
                ...
                <tbody>
                  ...
                  <tr
                    v-else-if="
                      !releasedLoading && filteredReleased.length === 0
                    "
                  >
                    <td colspan="7" class="text-center py-5">
                      <div class="empty-state">
                        ...
                        <p class="mt-2 text-muted">
                          {{
                            releasedSearch || releasedDocTypeFilter
                              ? "No documents match your filters"
                  ...
                  <tr v-for="(doc, index) in paginatedReleased" :key="doc.id">
                    <td class="text-center">
                      <span class="row-number">{{
                        (releasedCurrentPage - 1) * releasedPerPage + index + 1
                      }}</span>
                    </td>
                    <td>
                      <span class="tracking-number">{{
            ...
            <div class="pagination-wrapper" v-if="releasedTotalPages > 1">
              <div class="pagination-info">
                Showing {{ releasedStartItem }} to {{ releasedEndItem }} of
                {{ filteredReleased.length }} entries
              </div>
              <div class="pagination-buttons">
                <button
                  @click="releasedCurrentPage = 1"
                  :disabled="releasedCurrentPage === 1"
                  class="page-btn"
                >
                  <i class="bi bi-chevron-double-left"></i></button
                ><button
                  @click="releasedCurrentPage--"
                  :disabled="releasedCurrentPage === 1"
                  class="page-btn"
                >
                  <i class="bi bi-chevron-left"></i></button
                ><button
                  v-for="page in releasedDisplayedPages"
                  :key="page"
                  @click="releasedCurrentPage = page"
                  :class="[
                    'page-btn',
                    { active: releasedCurrentPage === page },
                  ]"
                >
                  {{ page }}</button
                ><button
                  @click="releasedCurrentPage++"
                  :disabled="releasedCurrentPage === releasedTotalPages"
                  class="page-btn"
                >
                  <i class="bi bi-chevron-right"></i></button
                ><button
                  @click="releasedCurrentPage = releasedTotalPages"
                  :disabled="releasedCurrentPage === releasedTotalPages"
                  class="page-btn"
                >
                  <i class="bi bi-chevron-double-right"></i>
...
export default {
  ...
  computed: {
    ...
    documentTypes() {
      const types = new Set();
      const allDocs = [
        ...this.inProgressDocuments,
        ...this.forReleaseDocuments,
        ...this.releasedDocuments,
      ];
      allDocs.forEach((doc) => {
    ...
    filteredInProgress() {
      let filtered = [...this.inProgressDocuments];
      if (this.inProgressDocTypeFilter)
        filtered = filtered.filter(
          (doc) => doc.document_type === this.inProgressDocTypeFilter
        );
      if (this.inProgressSearch) {
        const query = this.inProgressSearch.toLowerCase();
        filtered = filtered.filter(
          (doc) =>
    ...
    paginatedInProgress() {
      const start = (this.inProgressCurrentPage - 1) * this.inProgressPerPage;
      return this.filteredInProgress.slice(
        start,
        start + this.inProgressPerPage
      );
    },
    inProgressTotalPages() {
      return (
        Math.ceil(this.filteredInProgress.length / this.inProgressPerPage) || 1
      );
    },
    inProgressStartItem() {
      return this.filteredInProgress.length === 0
        ? 0
        : (this.inProgressCurrentPage - 1) * this.inProgressPerPage + 1;
    },
    inProgressEndItem() {
      return Math.min(
        this.inProgressCurrentPage * this.inProgressPerPage,
        this.filteredInProgress.length
      );
    },
    inProgressDisplayedPages() {
      const pages = [];
      const total = this.inProgressTotalPages;
      const current = this.inProgressCurrentPage;
      const delta = 2;
      for (let i = 1; i <= total; i++) {
        if (
          i === 1 ||
    ...
    filteredForRelease() {
      ...
      if (this.forReleaseDocTypeFilter)
        filtered = filtered.filter(
          (doc) => doc.document_type === this.forReleaseDocTypeFilter
        );
    ...
    paginatedForRelease() {
      ...
      return this.filteredForRelease.slice(
        start,
        start + this.forReleasePerPage
      );
    },
    forReleaseTotalPages() {
      return (
        Math.ceil(this.filteredForRelease.length / this.forReleasePerPage) || 1
      );
    },
    forReleaseStartItem() {
      return this.filteredForRelease.length === 0
        ? 0
        : (this.forReleaseCurrentPage - 1) * this.forReleasePerPage + 1;
    },
    forReleaseEndItem() {
      return Math.min(
        this.forReleaseCurrentPage * this.forReleasePerPage,
        this.filteredForRelease.length
      );
    },
    ...
    filteredReleased() {
      ...
      if (this.releasedDocTypeFilter)
        filtered = filtered.filter(
          (doc) => doc.document_type === this.releasedDocTypeFilter
        );
    ...
    releasedTotalPages() {
      return (
        Math.ceil(this.filteredReleased.length / this.releasedPerPage) || 1
      );
    },
    releasedStartItem() {
      return this.filteredReleased.length === 0
        ? 0
        : (this.releasedCurrentPage - 1) * this.releasedPerPage + 1;
    },
    releasedEndItem() {
      return Math.min(
        this.releasedCurrentPage * this.releasedPerPage,
        this.filteredReleased.length
      );
    },
